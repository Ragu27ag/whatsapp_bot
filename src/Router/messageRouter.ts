...
import { uploadMedia } from "../service/img_upload.service";


let messageRouters = express.Router();

const token = process.env.TOKEN;

console.log("tokend", token);

messageRouters.post("/webhook", async (req: Request, res: Response) => {
  try {
    ...
    if (body_param.entry[0].changes[0].value.contacts.length > 0) {
      ...
      if (
        ...
        let message: string = body_param.entry[0].changes[0].value.messages[0].text.body;
        console.log('messages', message);
        if (message.trim() === 'image') {
          console.log('imag');
          let image_id = await uploadMedia();
          let ress = await axios.post("https://graph.facebook.com/v20.0/" + phon_no_id + "/messages?access_token=" + token, {
          ...
          try {
            let ress = await axios.post("https://graph.facebook.com/v20.0/" + acct_no + "/message_templates", {
              name: "appoint_inder",
              language: "en_US",
              category: "MARKETING",
              components: [
